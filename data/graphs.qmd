...
quarto_rstudio_dls <- read.csv("quarto_rstudio_dls.csv", col.names = c("date", "downloads"))
quarto_rstudio_dls$pkg <- "quarto"

quarto_gh_dls <- read.csv("quarto_gh_downloads.csv", col.names = c("date", "name", "downloads"))
...
quarto_gh_dls %>% 
  group_by(date) %>% 
  summarise(downloads = sum(downloads)) %>% mutate(pkg = "quarto") %>%
  bind_rows(quarto_rstudio_dls) -> quarto_dls
