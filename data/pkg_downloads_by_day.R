...
write.csv(pydf, "data/pypi_stats.csv", row.names = FALSE)
...
write.csv(rpkgs, "data/rpkgs.csv", row.names = FALSE)

#tidyverse packages
tidyvpkgs <- cran_downloads(tidyverse_packages(FALSE), from = "2017-01-01")
write.csv(tidyvpkgs, "data/tidyverse_pkgs.csv", row.names = FALSE)
...
write.csv(tidympkgs, "data/tidymodels_pkgs.csv", row.names = FALSE)
...
cran_downloads(setdiff(rlib_repos$name, notcran), from = "2017-01-01") %>% 
  bind_rows() -> rlib_data
write.csv(rlib_data, "data/rlib_pkgs.csv", row.names = FALSE)

#r connectivity
rcondf <- cran_downloads(c("sparklyr", "tensorflow", "keras", "odbc", "reticulate"), from = "2017-01-01")
write.csv(rcondf, "data/rcon_pkgs.csv", row.names = FALSE)
